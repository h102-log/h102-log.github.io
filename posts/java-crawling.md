...
여기서 바로 **Jsoup의 치명적인 단점**이 드러납니다.

Jsoup은 최초에 서버로부터 전달받은 정적인 HTML 소스코드만 파싱하기 때문에, 브라우저가 JavaScript를 실행한 이후에 변경되거나 추가된 동적인 데이터는 읽어오지 못하는 것입니다.
...
시작 시점에 Playwright와 Browser를 1회 초기화하고,
...
이렇게 PlaywrightHolder 클래스를 만들어 적용하면,
...
public class PlaywrightHolderUsageExample {
    public static void main(String[] args) {
        ...
        try {
            ...
            // 요청 1건 처리 예시: 매 요청마다 withPage를 통해 안전하게 새 Page를 사용
...
다만 해당 방식은 단일 스레드 환경에서는 문제가 없지만, 멀티 스레드 환경에서는 동시성 이슈가 발생할 수 있습니다.

동시에 여러 요청이 들어올 때, 
하나의 Playwright 인스턴스를 공유하는 구조에서는 페이지 간 간섭이 발생할 수 있기 때문입니다.
이런 경우에는 Playwright 인스턴스를 스레드별로 관리하거나, 
요청 큐를 만들어 순차적으로 처리하는 등의 추가적인 설계가 필요합니다.

---

## 마무리

정리하면, 크롤링 방식은 "데이터가 정적인가, 동적인가"에 따라 선택하는 것이 가장 중요합니다.

1. 정적 HTML 중심이라면, 가볍고 빠른 Jsoup이 효율적입니다.
2. JavaScript 렌더링 이후 데이터까지 필요하다면, Playwright 같은 브라우저 자동화 도구가 적합합니다.

결국 핵심은 기술 자체보다, 대상 페이지의 렌더링 방식과 서비스 트래픽 특성에 맞는 구조를 선택하는 것입니다.