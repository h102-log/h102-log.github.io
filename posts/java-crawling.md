...
다만 해당 방식은 단일 스레드 환경에서는 문제가 없지만, 멀티 스레드 환경에서는
동시성 이슈가 발생할 수 있습니다.
동시에 여러 요청이 들어올 때, 하나의 Playwright 인스턴스를 공유하는 구조에서는 페이지 간 간섭이
발생할 수 있기 때문입니다.
이런 경우에는 Playwright 인스턴스를 스레드별로 관리하거나, 요청 큐를 만들어 순차적으로
처리하는 등의 추가적인 설계가 필요합니다.
