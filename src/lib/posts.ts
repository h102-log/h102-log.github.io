...
function clampPostImageWidth(width: number) {
  return Math.max(MIN_POST_IMAGE_WIDTH, Math.min(MAX_POST_IMAGE_WIDTH, Math.round(width)));
}

function clampPostImageHeight(height: number) {
  return Math.max(MIN_POST_IMAGE_HEIGHT, Math.min(MAX_POST_IMAGE_HEIGHT, Math.round(height)));
}
...
function rehypeApplyImageWidthFromTitle() {
  return (tree: HtmlNode) => {
    visitHtmlTree(tree, (currentNode) => {
      ...
      const titleValue = typeof currentNode.properties?.title === 'string'
      ...
      const parsedAlign = alignMatch ? (alignMatch[1].toLowerCase() as 'left' | 'center' | 'right') : undefined;

      if (!parsedWidth && !parsedHeight && !parsedAlign) {
        return;
      }

      const existingStyle = typeof currentNode.properties?.style === 'string'
      ...
      const sizeStyleParts = [
        ...
        parsedAlign === 'center' && parsedWidth ? 'display: block; margin-left: auto; margin-right: auto' : '',
      ].filter(Boolean);
...
function rehypeApplyAttachmentDownload() {
  return (tree: HtmlNode) => {
    visitHtmlTree(tree, (currentNode) => {
      ...
      const hrefValue = typeof currentNode.properties?.href === 'string'
        ? currentNode.properties.href
        : '';
      const titleValue = typeof currentNode.properties?.title === 'string'
        ? currentNode.properties.title
        : undefined;
      const hasDownloadToken = typeof titleValue === 'string' && /\bdownload\b/i.test(titleValue);
      ...
      const titleWithoutDownloadToken = typeof titleValue === 'string'
...
function getSharedTagCount(leftTags: string[] | undefined, rightTags: string[] | undefined) {
  if (!leftTags || !rightTags || leftTags.length === 0 || rightTags.length === 0) {
    return 0;
  }
...
export function getPostNavigation(id: string): PostNavigation {
  ...
  const previousPost = currentPostIndex < allPosts.length - 1
  ...
  const relatedPosts = allPosts
    ...
    .map((post) => ({
      ...
      dateDistance: Math.abs(getDateValue(currentPost.date) - getDateValue(post.date)),
    }))
    .sort((left, right) => {
